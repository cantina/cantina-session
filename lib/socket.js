...
exports.fromEngineSocket = function(options) {
  ...
  return function(socket, callback) {
    ...
    // grab the session cookie value and check the signature
    var rawCookie = cookie.parse(socket.transport.headers.cookie || "")[options.key];
    ...
    socket.sessionID = unsignedCookie;

    if (!socket.sessionID) {
      return callback();
    }

    options.store.load(socket.sessionID, function(err, session) {
      if (err) {
        return callback(err);
      }
